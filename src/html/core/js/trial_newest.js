...
function checkPolitics(once)
{
...

	{
		...
		{ 
			...
			$.post("core/DataWrangler.php", {"page":"polform", "userid":userid, "party":party });
...
function checkPolSurvey(once) // added iden as an input
{
	...
	*/
	...
	{   
		userid = $("#userid").html();
		$.post("core/DataWrangler.php", {"page":iden, "userid":userid, "data":qdata});
		$(wrapper).hide(500); 
		$('#error_popup').dialog( "close" );
...
function selfLabCheck(once)
{
	...
	else
	{
		...
		$('#error_popup').dialog( "close" );
		$.post("core/DataWrangler.php", 
		{"page":"aspect_labs", "userid":userid, 
			"data": self_labels
		});
		ShowGenaspect();
	}
		...
		$('#error_popup').dialog( "option", "buttons", [
			{
				...
				$( this ).dialog( "close" );
    			$.post("core/DataWrangler.php", {"page":"panas", "userid":userid, "data":panas_data});
    			if(stage==1)
    			{
