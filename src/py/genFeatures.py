...
def genPartyWordLists(savefiles):
	...
	conn = pymysql.connect(host='wmason.mgnt.stevens-tech.edu', port=3306, user='groupid', passwd='LetsPublish!', db='group_identity')
	cur = conn.cursor()

	cur.execute('SELECT survey.party, survey.Id, TweetText FROM survey JOIN tweet WHERE survey.party = "'+party1+'" OR survey.party = "'+party2+'" AND survey.Id=tweet.UserId ORDER BY survey.Id;')
	
	# This is the list of users & tweets in party1
	party_tweets=cur.fetchall()
	
	all_words=[[],[]]
	words_by_user=[{},{}]
	for tweet in party_tweets:
		u_party = tweet[0] == party1 and 0 or 1
		user = tweet[1]
		words = tweet[2].split(' ')
		if user in words_by_user:
			words_by_user[u_party][user].extend(words)
		else:
			words_by_user[u_party][user] = words
		all_words[u_party].extend(words)
...
if __name__ == "__main__":
	...
	loadfiles = int(vars(pargs)['loadfiles'])
	savefiles = vars(pargs)['savefiles']

	if loadfiles == 0:
		all_words,words_by_user = genPartyWordLists(savefiles)
		words,stems,hashes,bigrams,trigrams,endings = extractTerms(all_words,words_by_user, savefiles)
		UserScores(words,stems,hashes,bigrams,trigrams,endings, savefiles)
	elif loadfiles == 1:
		fh1=open('WordLists.pkl','r')
		all_words=cPickle.load(fh1)
		fh1.close()

		fh2=open('WordsByUser.pkl','r')
		words_by_user=cPickle.load(fh22)
		fh2.close()
		words,stems,hashes,bigrams,trigrams,endings = extractTerms(all_words,words_by_user, savefiles)
		UserScores(words,stems,hashes,bigrams,trigrams,endings, savefiles)
	elif loadfiles == 2:
