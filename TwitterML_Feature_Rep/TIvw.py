...
for i in range(len(seed_list[0])):
	vwtrain.append('0 |follow ' + ':1 '.join(seed_follow[0][i]) + ':1 |tweet ' + ' '.join(seed_list[0][i]))

for i in range(len(dev_list[0])):
	vwtrain.append('0 |follow ' + ':1 '.join(dev_follow[0][i]) + ':1 |tweet ' + ' '.join(dev_list[0][i]))

for i in range(len(seed_list[1])):
	vwtrain.append('1 |follow ' + ':1 '.join(seed_follow[1][i]) + ':1 |tweet ' + ' '.join(seed_list[1][i]))

for i in range(len(dev_list[1])):
	vwtrain.append('1 |follow ' + ':1 '.join(dev_follow[1][i]) + ':1 |tweet ' + ' '.join(dev_list[1][i]))

vwtest = []
for i in range(len(test_list[0])):
	vwtest.append('0 |follow ' + ':1 '.join(test_follow[0][i]) + ':1 |tweet ' + ' '.join(test_list[0][i]))

for i in range(len(test_list[1])):
	vwtest.append('1 |follow ' + ':1 '.join(test_follow[1][i]) + ':1 |tweet ' + ' '.join(test_list[1][i]))
